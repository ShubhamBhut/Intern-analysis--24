...
st.markdown(" ##### Here Stipend median is 27000, which is good. Also almost 64% students have paid internships which also good. Comparing with CS, these stats seem much better. To be honest rest branches are just terrible. God's grace I don't have their data.")
...
#st.plotly_chart(fig)
st.markdown(" As shown above, the average stipend is 36600 and average GPA is 7.66. However again, the reason behind this is 0 GPA of failed student. These failed students are pulling average GPA down. That means all the passed students must have high GPA, more than 8. let's check")
...
st.markdown(' Now look, average CGPA for all passed students is 9, awesome. Competitive! Have a look at Stipend now - its 41000. Nice, though this number is also pushed by top high stipend peeps.')
...
st.markdown("Average oncampus stipend is 64000, incredible, it's pushed though but still it's really good. Credits due here to ICC, TnP and students who kept GPA high")

df_offCampus = df.loc[df['Mode'] == "SELF"]
st.markdown("###### All OffCampus: ")
st.write(df_offCampus[['Stipend', 'CGPA', 'SGPA']].describe()[:3])
st.markdown("So average stipend of 42 offcampus students is 4000. But let's check after removing jugaadis")

df_offCampus = df_offCampus[df_offCampus['Stipend'] > 0]
st.markdown("###### All paid OffCampus:")
st.write(df_offCampus[['Stipend', 'CGPA', 'SGPA']].describe()[:3])
st.markdown("Now have a look, average stipend for offcampus is now 11000, still shit. and also only 15 students have paid offcampus interns, so there are 27 jugaadis in IT branch.")
...
st.markdown("""Here also some students with RE have paid interns, but the stipend is peanuts compare to CS students with RE. In my opinion when recruiters don't hire based on GPA, the key thing that matter is skill then(just pointing it out here). The graph clearly shows oncampus domination here.
...
st.markdown("Don't get confused, the fracrion of a bar indicated a single stipend value. A Whole bar represnts sum of all stipends for that CGPA. We don't see any bloodbath here, even tiniest rectangle of oncampus is of 27k here. Cheers to IT peeps")
...
st.pyplot(fig)
st.markdown("Above graph is quite generalized, but still gives a lot of insight. look at uptrend,the clear rise of Stipend with GPA. Common observation says more GPA => more Stipend. So study and get GPA pal, kuchh na rakha dancing, drawing ya gaming me.")
...
st.markdown(f"total males with stipend > 0 are {df_male[df_male['Stipend'] > 0].count()[0]} and {100*(63/101)}% of all males")
st.markdown(f"total females with stipend > 0 are {df_female[df_female['Stipend'] > 0].count()[0]} and {100*(20/27)}% of all females")
...
st.markdown("""Here average female CGPA is better than male counterpart, so higher average stipend is seen. But compare to gap in CGPA(0.5), the gap in stipend(21k gap) is just huge. so if there is no gender bias then every 0.1 GPA = 4k. Doesn't seem very true. Let's check for only passed students""")
...
st.markdown("""Now GPA gap is only 0.2 but CGPA gap is 24,000. Interesting....
...
st.markdown("Again similar to CS. Females generally have high stipend interns. A lot of males with low stipend brought the average down. One thing I relly like about IT is that everyone has good intern, some has great, some has awesome, but everyone has good intern.")
...
st.plotly_chart(fig, use_container_width=True)
st.markdown("As we saw previously, females have little edge with CGPA and that really paid off as good stipend")
...
st.markdown("""Above chart again gives interesting insights, Females have almost always stayed above males irrespective of GPA except offcampus range (8.4-8.7, it more like 9.6-8.7 here). Let's confirm these observations with other plots to look in depth""")
...
fig.update_layout(template="plotly_dark")
st.plotly_chart(fig, use_container_width=True)
st.markdown("Nothing is as happy as it seems, especially for boys. Median stipend is only 5000 while for girls it is 15,000. just a little 3x gap. Also the distribution after median shows insane stipend gap.")
...
st.markdown("""Just like CS, the offcampus is dominated by males, but in case of IT the vast majority of interns are oncampus so that matters much more here. And female stipend stats have wrecked male stipend stats. Female stipend median is 75k, just insane and awesome.""")

st.markdown("We saw even bigger stipend gap here in favour of females. So does being a female helps ? let's make algorithm to decide.")
...
st.markdown("Female regression line has always stayed above male line and also unlike CS, here the stipend gap is increasing with increase in CGPA.")
...
st.markdown("""0. IT interns were just dominated by oncampus and they perfomed much better than CS overall.

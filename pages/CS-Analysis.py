...
# st.plotly_chart(fig)
st.markdown(
    " As shown above, the average stipend is 21880 and average GPA is 5.18. However the reason behind this is 0 GPA of failed student. These failed students are pulling average GPA down. That means all the passed students must have high GPA, more than 8. let's check"
)
...
st.markdown(
    " It seems that Stipend wise oncampus is a much better option with average Stipend around 44000. I will be honest, there are some factors which has pushed this number, but stil oncampus stipend is good."
)
...
st.markdown(
    "It seems total offcampus internships are 40 with mean Stipend 7500. Total shit, but let's look after removing these jugaadis."
)
...
st.write(df_offCampus[["Stipend", "CGPA", "SGPA"]].describe()[:3])
st.markdown(
    "Now have a look, average stipend for offcampus is now 37500, not bad. But look at frequency(count), only 8 people have offcampus paid internships, that means around 32 people in CS are jugaadi, shameful. Also it requires skills to get offcampus paid intern."
)
...
st.markdown(
    "Don't get confused, the fracrion of a bar indicated a single stipend value. A Whole bar represnts sum of all stipends for that CGPA. Look at range of stipends varying with CGPA, at 9.2 you have mostly good stipends, while RE and 8.4 is bloodbath"
)
...
st.pyplot(fig)
st.markdown(
    "Above graph is quite generalized, but still gives a lot of insight. look at uptrend, rise of Stipend with GPA. Now look at the slope after 9.4. After 9.4, the probability of good intern is excellent. So study and get GPA pal, kuchh na rakha dancing, drawing ya gaming me."
)
st.markdown(
...
st.markdown(
    f"total males with stipend > 0 are {df_male[df_male['Stipend'] > 0].count()[0]} and {100*(41/92)}% of all males"
)
st.markdown(
    f"total females with stipend > 0 are {df_female[df_female['Stipend'] > 0].count()[0]} and {100*(16/25)}% of all females"
)
...
st.plotly_chart(fig, use_container_width=True)
st.markdown(
    "Nothing is as happy as it seems, especially for boys. Median stipend is 0 while for girls it is 30,000. Also the distribution after median shows insane stipend gap is, boys q3 is same as girls median."
)
st.markdown(
...
5. Oncampus is great if get selected for almost all studens, have mostly higher stipend and Offcampus selection chances are super rare.
6. Generally Offcampus selected peeps are boys and with GPA range of 8.4-8.7 for previously discussed reasons.
7. Though girls group has low GPA, they still are in much better intern, so being a girl definitely helps by a lot. A lot varies as per GPA range
8. Highest stipend were secured by peeps with highest GPA, almost all boys, which pushed average stipend of boys data.
9. Girls didn't shine offcampus, but Oncampus, they did much better than average boys, couldn't find reasons except gender.
